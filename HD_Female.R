...
data <- read_excel("Data/CarolGubert_HD-FMT_female_data-5.xlsx")
...
## ---------------------------------------------------------------------------------------------------------
data.repeated.1<-data.new[,c(1:6,8:37,66:80,96:123)]
...
long_data.repeated.1<-long_data.repeated.1[order(long_data.repeated.1$ID,
                                                 long_data.repeated.1$BOX,long_data.repeated.1$Time),]
...
## ---------------------------------------------------------------------------------------------------------
FoodIntake.data<-data.new[,c(1:6,40:51)]
...
WaterIntake.data<-data.new[,c(1:6,55:65)]
...
## ---------------------------------------------------------------------------------------------------------
CFC.data<-data.new[,c(1:6,164:170)]
...
CFC.ex.data<-data.new[,c(1:6,171:180)]
...
## ---------------------------------------------------------------------------------------------------------
Ymaze.data<-data.new[,c(1:6,128:129)]
...
dep_vars <-c("Ymaze_index","Ymaze_Dist_trial1","NOR_RI_5min",
             "BrainWeight_W14","Digigait_PropelBrakeRatio_W14",
...
stargazer(Ymaze_index.lmm_,Ymaze_Dist_trial1.lmm_,
          NOR_RI_5min.lmm_,BrainWeight_W14.lmm_,Digigait_PropelBrakeRatio_W14.lmm_,
          type = "text",
          digits = 4,
          report = ('vc*p'),
          star.char = c("+", "*", "**", "***"),
...
## ---------------------------------------------------------------------------------------------------------
#subset(data, !is.na(Acetate_w14)) %>%
    #group_by(genotype, FMT, ATB) %>%
    #tally()
dep_vars <-c("Acetate_w14","Propionate_w14",
             "Isobutyrate_w14","Methylbutyrate_w14",
             "Acetate_w20","Propionate_w20","Isobutyrate_w20",
             "Methylbutyrate_w20")
...
stargazer(IL21_w14.lmm,IL7R_w14.lmm,
          type = "text",
          digits = 4,
          report = ('vc*p'),
          star.char = c("+", "*", "**", "***"),
          ...
          notes.append = FALSE,
          dep.var.labels.include  = FALSE,
          column.labels=c( "IL21-14","IL7R-14"),
          align=TRUE)



## ---------------------------------------------------------------------------------------------------------
dep_vars <-c("IL17A_w14", "IL17E_w14", "IFNg_w14","IL1b_w14","IL22_w14",
             "IL6_w14","TNFa_w14","IFNg_w20","IL17A_w20","IL17E_w20",
             "IL1b_w20","IL21_w20","IL22_w20","IL6_w20","TNFa_w20")
...
stargazer(IL17A_w14.lmm,IL17E_w14.lmm,
          IFNg_w14.lmm,IL1b_w14.lmm,IL22_w14.lmm,
          type = "text",
          digits = 4,
          report = ('vc*p'),
          star.char = c("+", "*", "**", "***"),
          ...
          notes.append = FALSE,
          dep.var.labels.include  = FALSE,
          column.labels=c("IL17A-14", "IL17E-14","IFNg-14","IL1b-14","IL22-14"),
          align=TRUE)

stargazer(IL6_w14.lmm,TNFa_w14.lmm,
          IFNg_w20.lmm,IL17A_w20.lmm,IL17E_w20.lmm,
          type = "text",
          digits = 4,
          report = ('vc*p'),
          star.char = c("+", "*", "**", "***"),
          ...
          notes.append = FALSE,
          dep.var.labels.include  = FALSE,
          column.labels=c("IL6-14","TNFa-14",
                          "IFNg-20","IL17A-20","IL17E-20"),
          align=TRUE)

stargazer(IL1b_w20.lmm,IL21_w20.lmm,
          IL22_w20.lmm,IL6_w20.lmm,TNFa_w20.lmm,
          type = "text",
          digits = 4,
          report = ('vc*p'),
          star.char = c("+", "*", "**", "***"),
          ...
          notes.append = FALSE,
          dep.var.labels.include  = FALSE,
          column.labels=c("IL1b-20",
             "IL21-20","IL22-20","IL6-20","TNFa-20"),
          align=TRUE)



## ---------------------------------------------------------------------------------------------------------
data_clasping <- data.new[,c(1:6,83:95)] # extract the columns of data that will become Y
...
## ----barplot_clas_count_fe, echo = FALSE, fig.cap ='Bar plot for clasping over the 12 weeks', fig.pos="H",  fig.width = 10, fig.height = 9,  out.width = "1\\textwidth"----
...
## ----barplot_clas_per_fe, echo = FALSE, fig.cap ='Bar plot for clasping over the 12 weeks', fig.pos="H",  fig.width = 10, fig.height = 9,  out.width = "1\\textwidth"----
...
## ----barplot_clas_per_fe_hd, echo = FALSE, fig.cap ='Bar plot for clasping over the 12 weeks for HD mice', fig.pos="H",  fig.width = 10, fig.height = 9,  out.width = "1\\textwidth"----
ggplot(data.frame(as.matrix(t1)), aes(fill=Var2, y=Freq, x=Var1)) + 
    geom_bar( stat="identity", position = position_stack(reverse = TRUE)) +
    scale_fill_viridis(discrete = T)+ylab("Percent")+xlab("Time")+ 
  guides(fill=guide_legend(title="Clasping Score"))
...
## ----barplot_clas_per_fe_wt, echo = FALSE, fig.cap ='Bar plot for clasping over the 12 weeks for WT mice', fig.pos="H",  fig.width = 10, fig.height = 9,  out.width = "1\\textwidth"----
ggplot(data.frame(as.matrix(t1)), aes(fill=Var2, y=Freq, x=Var1)) + 
    geom_bar( stat="identity", position = position_stack(reverse = TRUE)) +
    scale_fill_viridis(discrete = T)+ylab("Percent")+xlab("Time")+ 
  guides(fill=guide_legend(title="Clasping Score"))
...
Clasping.clmm = clmm(Clasping~genotype + ATB + FMT + Time+(1|BOX), 
           data=long_clasping, Hess=TRUE)
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(Rotarod.lmm, pairwise ~ genotype|Treatment, 
        at = list(Time = c(1:13)))
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results for genotype given Treatment in Rotarod") %>%
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(FecalOutput.lmm, pairwise ~ genotype*Treatment, 
        at = list(Time = c(1:13)))
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results Post-hoc results for genotype*Treatment in Fecal Output") %>%
  kable_styling(latex_options = c("repeat_header"))%>%
  column_spec(6,bold = that_cell,
              color =  ifelse(that_cell,"red","black"))%>%
  column_spec(1,color =  ifelse(that_cell,"red","black"))
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(FecalOutput.lmm, pairwise ~ genotype*Treatment|Time, 
        at = list(Time = c(1:13)))
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results Post-hoc results for genotype*Treatment given Time in Fecal Output") %>%
  kable_styling(latex_options = c("repeat_header"))%>%
  column_spec(7,bold = that_cell,
              color =  ifelse(that_cell,"red","black"))%>%
  column_spec(1,color =  ifelse(that_cell,"red","black"))
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(Ymaze.lmm, pairwise ~ genotype*Arm|Treatment)
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results for genotype and arm given Treatment in Ymaze") %>%
  kable_styling(latex_options = c("repeat_header"))%>%
  column_spec(7,bold = that_cell,
              color =  ifelse(that_cell,"red","black"))%>%
  column_spec(1,color =  ifelse(that_cell,"red","black"))
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(Propionate_w20.lmm, pairwise ~ Treatment|genotype)
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results for Treatment given genotype in Propionate") %>%
  kable_styling(latex_options = c("repeat_header"))%>%
  column_spec(7,bold = that_cell,
              color =  ifelse(that_cell,"red","black"))%>%
  column_spec(1,color =  ifelse(that_cell,"red","black"))
...
## ---------------------------------------------------------------------------------------------------------
x<-emmeans(IFNg_w14.lmm, pairwise ~ genotype|Treatment)
df<-data.frame(x$contrasts)
that_cell<-df$p.value<0.05
kbl(x$contrasts, longtable=T, linesep = "", booktabs = T, digits = c(4,4,4,4,4,4),
    format = "latex",caption = "Post-hoc results for genotype given Treatment in IFNg") %>%
  kable_styling(latex_options = c("repeat_header"))%>%
  column_spec(7,bold = that_cell,
              color =  ifelse(that_cell,"red","black"))%>%
  column_spec(1,color =  ifelse(that_cell,"red","black"))
